# Function to run the sampler through each of the two sigma parameters
...
# currentSigma - An integer scalar used to select which sigma parameter the sampler will run for.
...
sigmaSampler <- function (model,
  ...
  # Create a vector to hold the prposed sigma value
  newValues <- currentValues

  # Generate new value for current sigma
  sigmaStar <- rtruncnorm(n = 1,
                          a = 0,
                          b = Inf,
                          mean = currentValues[currentSigma],
                          sd = proposalSD)

  newValues[currentSigma] <- sigmaStar

  # Calculate the numerator and denominator for the acceptance ratio
  numerator <- (log(dtruncnorm(x = newValues[currentSigma],
                               a = 0,
                               b = Inf,
                ...
                + log(dtruncnorm(x = currentValues[currentSigma],
                                 a = 0,
                                 b = Inf,
                                 mean = newValues[currentSigma],
                                 sd = proposalSD)))

  denominator <- (log(dtruncnorm(x = currentValues[currentSigma],
                                 a = 0,
                                 b = Inf,
                                 mean = priorParameters[1],
                                 sd = priorParameters[2]))
                  ...
                  + log(dtruncnorm(x = newValues[currentSigma],
                                   a = 0,
                                   b = Inf,
                                   mean = currentValues[currentSigma],
                                   sd = proposalSD)))
  ...
  # Determine if the proposed sigma should be accepted.
